...
@given(u'we have a python file `{filename}` with the following statements')
@given(u'we have a simply python file `{filename}` with the following statements')
...
@when(u'we transform this file with `nbsynconvert to-notebook {source_file} {target_file}`.')
...
@then(u'a file `{filename}` should be created, containing a Synapse Notebook.')
...
@then(u'the created notebook should contain one cell.')
def step_impl(context: Context):
    context.execute_steps('Then the notebook should contain 1 cells.')


@then(u'the notebook should contain only `{count}` cells.')
@then(u'the notebook should contain `{count}` cells.')
def step_impl(context, count):
    assert len(context.notebooks[-1]['properties']['cells']) == int(count)

    context.cells = [ ''.join(cell.source) for cell in context.notebooks[-1]['properties']['cells']]


@then(u'the first cell should contain')
def step_impl(context: Context):
    assert_cell_content(context, 0, context.text)

@then(u'the second cell should contain')
def step_impl(context):
    assert_cell_content(context, 1, context.text)


@then(u'the cell should contain the content from the input file.')
def step_impl(context):
