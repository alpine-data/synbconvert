...
def step_impl(context) -> None:  # noqa: F811
    ...
    exec_text = (
        f"Given we have a Synapse notebook file with the name `{notebook_name}`\n"
...
def assert_cell_hidden_status(
    context: Context, index: int, hidden_status: bool
) -> None:
    ...
    assert (
        source_hidden == hidden_status
    ), "The cell source_hidden status is not correct."
    assert (
        outputs_hidden == hidden_status
    ), "The cell outputs_hidden status is not correct."
